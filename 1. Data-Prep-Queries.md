...
DROP TABLE IF EXISTS [s3_load_drr_a]

CREATE TABLE [s3_load_drr_a] (
...
ym_cte
  ...
  AS (
    SELECT 
	CONVERT(VARCHAR(10), a.[year_month], 20) 
       ,a.[item_id] 
       ,a.[keycust3]
...
```
DROP TABLE IF EXISTS [s3_load_partition_smooth_drr_ab];

CREATE TABLE [s3_load_partition_smooth_drr_ab] (
   [year_month] VARCHAR(10)
  ,[item_id] VARCHAR(54)
  ,[keycust4] VARCHAR(54)
  ,[frcst_qty] NUMERIC(10, 0)
  ...
  AS ( 
    ...
    FROM (
	   ...
	   [aws_base].[item_id]
          ,[aws_base].[keycust4] 
          ,SUM( SUM([frcst_qty]) ) 
	       OVER (PARTITION BY [item_id], [keycust4]) AS [frcst_qty_a]
        FROM [aws_base]
		JOIN [last_6_month cte]
		  ON [aws_base].[year_month] = [last_6_month cte].[year_month]
...
[ym_cte] (  
  ...
  AS (
    SELECT 
      ...
      ,a.[item_id] 
      ,a.[keycust4]
      ,CASE WHEN SUM(ISNULL(a.[frcst_qty], 0)) <= 0
	        THEN 0 ELSE SUM(ISNULL(a.[frcst_qty], 0))
			END
    FROM [aws_base] a
    JOIN [items_cte] b 
	  ON a.[item_id] = b.[item_id]
	...
	WHERE [line_code] IN ('DRR')
	  AND [pop_code_legacy]  IN ('a', 'b')
	  AND a.[year_month] NOT between DATEADD(MM,DATEDIFF(MM,0,Getdate()),0) and DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) +1, 0))
	  AND [active_flag] != 'I'
	  AND a.[keycust4] NOT IN ('Warren Distributing', 'Automotive Brake', 'Automotive Hard Parts', 'e&o')
		  --AND a.[keycust4] NOT LIKE 'Intra%'
     GROUP BY
	a.[year_month]
       ,a.[item_id] 
       ,a.[keycust4])
...
INSERT INTO [s3_load_partition_smooth_drr_ab]
     ( [year_month]  
      ,[item_id]
      ,[keycust4]  
      ,[frcst_qty]
...
SELECT
  ...
  ,CASE WHEN 
     ABS ( SUM([cte_metrics].[sum_frcst_qty]) - SUM([cte_metrics].[avg_frcst_qty]) ) 
       > 
     ABS ( SUM([cte_metrics].[stddev_frcst_qty]) * 2 )
