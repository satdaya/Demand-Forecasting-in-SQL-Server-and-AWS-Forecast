...
```
The query above was only smoothing the April 2020 COVID dip. Past year load orders (not scheduled to repeat) were still showing up pronounced in the forecast. The query below takes any variations sets the frcst_qty to the mean when the frcst_qty is greater than 2x standard deviation.
